...
function Hero() {
  ...
  return (
    <section id="hero" className="mx-auto max-w-5xl px-6 pb-20 pt-36">
      <div className="grid items-center gap-12 md:grid-cols-[1.4fr_1fr]">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, ease: EASE }}>
          ...
          <h1 className={`${SERIF} font-semibold leading-[1.04] tracking-[-0.02em] ${INK}`} style={{ fontSize: 'clamp(2.8rem, 6.5vw, 5rem)' }}>
            {PROFILE.firstName} {PROFILE.lastName}
          </h1>
          <p className="mt-4 min-h-[1.9rem] text-[1.2rem] text-stone-600">
            {displayed}
            <span className={`ml-0.5 font-mono text-blue-700 ${done ? 'opacity-0' : 'animate-cursor-blink'}`}>|</span>
          </p>
          <p className="mt-6 max-w-[520px] text-[1.05rem] leading-[1.85] text-stone-600">{PROFILE.summary}</p>

          <div className="mt-9 flex flex-wrap items-center gap-3">
            <button onClick={() => scrollTo('#work')} className="inline-flex items-center gap-2 rounded-full bg-stone-900 px-6 py-3 text-[14px] font-medium text-stone-50 transition-colors hover:bg-blue-700">
              View Atlas case study <ArrowRightIcon width={15} height={15} />
            </button>
            <button onClick={() => scrollTo('#contact')} className="rounded-full border border-stone-300 px-6 py-3 text-[14px] font-medium text-stone-800 transition-colors hover:border-stone-900">
              Get in touch
            </button>
          </div>
        </motion.div>

        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8, ease: EASE, delay: 0.1 }} className="relative mx-auto">
          <div className="absolute -bottom-3 -right-3 h-full w-full rounded-2xl border border-blue-700/30" />
          <img src={PROFILE.photo} alt={PROFILE.name} className="relative h-72 w-64 rounded-2xl border border-stone-200 object-cover shadow-sm sm:h-80 sm:w-72" />
        </motion.div>
      </div>
    </section>
  );
